...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        href: '/gestion/despachante',
        label: 'Despachante',
        sectionKey: 'despachante',
        icon: (
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  {
    // Va al final a propósito: es la red de seguridad, no un módulo de trabajo.
    // La ve cualquiera que pueda borrar algo (rol con escritura y alguna sección).
    section: 'Sistema',
    items: [
      {
        href: '/gestion/papelera',
        label: 'Papelera',
        writeOnly: true,
        icon: (
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function GestionLayoutClient({
  ...
  // Mismo criterio que canEdit + requireWrite en lib/perms.ts: admin siempre;
  // editor solo si tiene al menos una sección; viewer nunca.
  const puedeEscribir = isAdmin || (role === 'editor' && sections.length > 0)
  ...
  const navGroups = NAV_ITEMS
    ...
    .map((g: any) => ({
      ...
      items: g.items.filter((it: any) => {
        ...
        if (it.writeOnly) return puedeEscribir
